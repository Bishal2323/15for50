type Report = {
  ...
  trainingLoadSRPE: number; // Computed load
  ...
  // Legacy metrics (optional in new flow)
  painLevel?: number;    // 0-10
  localSoreness?: number; // 0-10
  ...
  kneeStabilityL: number; // 1-10
  kneeStabilityR: number; // 1-10
};
...
function makeRecommendation(violations: string[]): string {
  if (violations.includes('Workload Spike')) return 'Reduce training load and increase recovery.';
  if (violations.includes('High Training Load')) return 'Consider reducing training intensity or duration.';
  if (violations.includes('Symptom Persistence')) return 'Physio evaluation recommended.';
  if (violations.includes('Strength Imbalance')) return 'Add targeted strength work to correct imbalance.';
  if (violations.includes('Fatigue/Sleep')) return 'Prioritize sleep and recovery strategies.';
  if (violations.includes('Poor Recovery')) return 'Focus on sleep quality and stress management.';
  if (violations.includes('High Stress')) return 'Consider stress management techniques and lighter training.';
  if (violations.includes('Low Readiness')) return 'Reduce training intensity until readiness improves.';
  return 'Maintain current plan and monitor.';
}

export function computeRisk(reports: Report[]) {
  ...
  if (acwr > 1.5) violations.push('Workload Spike');
  ...
  if (kneeStreak >= 2) violations.push('Symptom Persistence');
  
  const strengthImbalance = Math.abs(latest.kneeStabilityL - latest.kneeStabilityR) >= 3;
  if (strengthImbalance) violations.push('Strength Imbalance');
  ...
  if (latest.stressLevel >= 80) violations.push('High Stress');
  
  if (latest.readinessToTrain <= 4 || latest.mood <= 4) violations.push('Low Readiness');
