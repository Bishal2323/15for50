...
async function resolveAthleteId(athleteEmail) {
    const athlete = await user_model_1.User.findOne({ email: athleteEmail });
    return athlete ? athlete._id : null;
}
// Helper function to validate report type permissions
function validateReportTypePermission(userRole, reportType) {
    const permissions = {
        daily: ['Athlete', 'Admin'],
        weekly: ['Coach', 'Admin'],
...
router.get('/athlete/:athleteId', auth_1.requireAuth, async (req, res) => {
    try {
        const { athleteId } = req.params;
        const requester = req.user;
        if (!mongoose_1.default.Types.ObjectId.isValid(athleteId)) {
        ...
        const riskFactor = await riskFactor_model_1.RiskFactor.findOne({
        ...
        res.json({
            ...
            data: {
                ...
                currentRiskFactors: {
                    strengthAsymmetry: ((riskFactor.strengthAsymmetry || []).slice(-1)[0]?.value ?? 0),
                    neuromuscularControl: ((riskFactor.neuromuscularControl || []).slice(-1)[0]?.value ?? 0),
                    anatomicalFixedRisk: ((riskFactor.anatomicalFixedRisk || []).slice(-1)[0]?.value ?? 0),
                },
                latest: {
                    ...
                    note: (riskFactor.notes || []).slice(-1)[0] || null,
                }
            }
        });
    }
...
router.get('/athlete/email/:athleteEmail', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const riskFactor = await riskFactor_model_1.RiskFactor.findOne({ athleteId });
        if (!riskFactor) {
            return res.status(404).json({ error: 'Risk factors not found for this athlete' });
        }
        res.json({
...
router.post('/submit', auth_1.requireAuth, async (_req, res) => {
...
router.get('/history/:athleteId', auth_1.requireAuth, async (req, res) => {
    try {
        const { athleteId } = req.params;
        ...
        const flatten = (metric, arr) => (arr || []).map(v => ({ metric, value: v.value, date: new Date(v.date), reportType: v.reportType }));
        let history = [
            ...flatten('workloadManagement', riskFactor.workloadManagement),
            ...flatten('mentalRecovery', riskFactor.mentalRecovery),
            ...flatten('strengthAsymmetry', riskFactor.strengthAsymmetry),
            ...flatten('neuromuscularControl', riskFactor.neuromuscularControl),
            ...flatten('anatomicalFixedRisk', riskFactor.anatomicalFixedRisk),
        ];
        // Apply filters
        if (reportType) {
...
router.get('/summary/:athleteId', auth_1.requireAuth, async (req, res) => {
    try {
        const { athleteId } = req.params;
